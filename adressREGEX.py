...
def extract_info(text):
    ...
    ulica_patterns = r"(?:\bul\b|\bul\.\b|ulic\w*)[ ]*[^\w\s]?[ ]*([a-ząćęłńóśźż0-9\s]+?)\s+(\d+\w*)[,. ]"
    dzialka_pattern = r"(?:\bdz\b|\bdz\.\b|dział\w*)[^\d]*(\d+(?:/\d+)?(?:[^\w]+\d+(?:/\d+)?)*).*"
    obreb_pattern = r"(?:ob\.|obr\.|obrę\w*)[^\d]*(\d+\w*)"
    ...
    if not results:
        ...
        obreb_pattern = r"(?:obr\.|obr[^\w]*)[^\d]*(\d+\w*)?"
...
#
#     "Montaż na działce dz. 178 obręb 100, ul. Spacerowa"
# ]
#
# # Wywołanie funkcji dla każdej linii
# for text in texts:
